...
- **The three manual workflows are `workflow_dispatch`-only, deliberately.** `test-set-lineup.yml` exercises a write against a real MFL team; `test-login-endpoint.yml` probes the deployed login endpoint; `probe-mfl-season.yml` asks MFL what it returns for a league in a season that may not exist. Never add a `schedule` trigger to any of them.
